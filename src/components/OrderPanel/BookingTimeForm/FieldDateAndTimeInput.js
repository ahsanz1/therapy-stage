...
import {
  ...
  getStartOf,
  initialVisibleMonth,
  convertTimeSlotsToTimestamps,
  convertTimeSlotsToTimestampsTimeEnds,
  calculateStartTimes,
  calculateEndTimes,
} from '../../../util/dates';
...
class FieldDateAndTimeInput extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      selectedTimeSlots: [], // Set to an empty array by default
    };
  ...
  onAddTimeSlot() {
    const { values, form, monthlyTimeSlots, timeZone, intl } = this.props;
    const { bookingStartDate, bookingStartTime, bookingEndDate, bookingEndTime } = values;

    // Check if all necessary fields are present
    if (bookingStartDate && bookingStartTime && bookingEndDate && bookingEndTime) {
      const newTimeSlot = {
      ...
      // Check if the new time slot is not already in the selected time slots array
      const isDuplicate = this.state.selectedTimeSlots.some(
        slot =>
          `${slot.startDate}_${slot.startTime}` ===
            `${newTimeSlot.startDate}_${newTimeSlot.startTime}` &&
          `${slot.endDate}_${slot.endTime}` === `${newTimeSlot.endDate}_${newTimeSlot.endTime}`
      );

      if (!isDuplicate) {
        // Update the selected time slots array in the state
        this.setState(
          prevState => ({
            selectedTimeSlots: [...prevState.selectedTimeSlots, newTimeSlot],
          }),
          () => {
            ...
            // Now, update the form fields similar to onBookingStartDateChange
        ...
        // Handle the case where the time slot is a duplicate (e.g., show an error message)
        console.warn('Duplicate time slot is not allowed.');
      }
    }
  }

  onRemoveTimeSlot(index) {
    // Update the selected time slots array in the state by removing the specified index
    this.setState(
      prevState => ({
        selectedTimeSlots: prevState.selectedTimeSlots.filter((_, i) => i !== index),
      }),
      () => {
        // Clear the form fields if there are no more selected time slots
  ...
  render() {
    ...
    const convertedTimeSlotsStart =
      timeSlotsOnSelectedDate &&
      timeSlotsOnSelectedDate?.length > 0 &&
      calculateStartTimes(timeSlotsOnSelectedDate, timeZone);
    const convertedTimeSlotsEnd =
      selectedTimeSlot && calculateEndTimes([selectedTimeSlot], timeZone);
    ...
    return (
      <div className={classes}>
            ...
            >
              {bookingStartDate ? (
                <>
                  ...
                  {convertedTimeSlotsStart &&
                    convertedTimeSlotsStart?.map(p => (
                      <option key={p.timeOfDay} value={p.timestamp}>
                        {localizedHourStrings(p.timeOfDay, intl)}
                      </option>
                    ))}
                </>
              ) : (
          ...
          <div className={css.field}>
            ...
            >
              {bookingEndTimeAvailable ? (
                <>
                  <option hidden>{'--:--'}</option>
                  {convertedTimeSlotsEnd &&
                    convertedTimeSlotsEnd?.map(p => (
                      <option
                        key={p.timeOfDay === '00:00' ? '24:00' : p.timeOfDay}
                        value={p.timestamp}
                      >
                        {localizedHourStrings(
        ...
        {/* <button type="button" onClick={this.onAddTimeSlot}>
          Add Time Slot
        </button> */}

        {/* Display the selected time slots */}
        <div className={css.selectedTimeSlots}>
          {this.state.selectedTimeSlots.map((slot, index) => (
            <div key={index} className={css.selectedTimeSlot}>
              <span>{`${slot.startDate} ${slot.startTime} - ${slot.endDate} ${slot.endTime}`}</span>
              <button type="button" onClick={() => this.onRemoveTimeSlot(index)}>
                Remove
              </button>
            </div>
          ))}
