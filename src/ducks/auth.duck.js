...
import { loginSignUpEvent } from '../util/datalayer-events';
...
export const login = (username, password) => (dispatch, getState, sdk) => {
  ...
  return sdk
    ...
    .then(response => {
      ...
      loginSignUpEvent('login', user_id, 'userId');
    })
...
export const signup = params => (dispatch, getState, sdk) => {
  ...
  return sdk.currentUser
    ...
    .then(response => {
      let user_id = response?.data?.data?.id?.uuid;
      loginSignUpEvent('sign_up', user_id, 'user_id');
