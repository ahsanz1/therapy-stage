...
export const pathByRouteName = (nameToFind, routes, params = {}) => {
  ...
  const hasEmptyName = pathParams && !pathParams?.organizedName;
  const pathParamswithName = hasEmptyName
    ? { ...pathParams, organizedName: 'no-name' }
    : pathParams;
  return toPathByRouteName(nameToFind, routes)(pathParamswithName);
};
...
export const canonicalRoutePath = (routes, location, pathOnly = false) => {
  ...
  if (isListingRoute) {
    ...
    const canonicalListingPathname = `/${parts[1]}/${parts[3]}/${parts[4]}`;
