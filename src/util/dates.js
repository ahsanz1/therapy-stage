...
export const convertTimeSlotsToTimestamps = (timeSlots, timeZone) => {
  if (!moment.tz.zone(timeZone)) {
    throw new Error('Time zones are not loaded into moment-timezone. Function uses time zones.');
  }

  const results = [];

  timeSlots.forEach(slot => {
    const startTime = moment.tz(slot?.attributes?.start, timeZone);
    const endTime = moment.tz(slot?.attributes?.end, timeZone);

    while (startTime?.isBefore(endTime)) {
      const timestamp = startTime.valueOf();
      const timeOfDay = startTime.format('HH:mm');
...
export const convertTimeSlotsToTimestampsTimeEnds = (timeSlots, timeZone) => {
  if (!moment.tz.zone(timeZone)) {
    throw new Error('Time zones are not loaded into moment-timezone. Function uses time zones.');
  }
  ...
  timeSlots?.forEach(slot => {
    const startTime = moment.tz(slot?.attributes?.start, timeZone);
    const endTime = moment.tz(slot?.attributes?.end, timeZone);
    // Ensure that startTime is one hour before endTime
    const adjustedStartTime = endTime.clone().subtract(1, 'hour');
    const timestamp = adjustedStartTime.valueOf();
    const timeOfDay = adjustedStartTime.format('HH:mm');
...
export const calculateStartTimes = (timeSlots, timezone) => {
  return timeSlots.map(timeSlot => {
    const startTime = moment.tz(timeSlot.attributes.start, timezone);
    const timestamp = startTime.valueOf(); // Get the timestamp
    const time = startTime.format('HH:mm'); // Format the time
...
export const calculateEndTimes = (timeSlots, timezone) => {
  return timeSlots.map(timeSlot => {
    // const startTime = moment.tz(timeSlot.attributes.start, timezone);
    const endTime = moment.tz(timeSlot.attributes.end, timezone);
    // const endTime = startTime.clone().add(1, 'hour'); // Add one hour to the start time
    const formattedEndTime = endTime.format('HH:mm'); // Format the end time
    const endTimeTimestamp = endTime.valueOf(); // Get the timestamp for the end time

    return {
      timestamp: endTimeTimestamp,
      timeOfDay: formattedEndTime,
    };
  });
};
