...
const GroupOfFieldCheckboxes = props => {
  const { id, className, name, options, mainDivClass } = props;
  return (
    <fieldset className={`${className} ${css[mainDivClass]}`}>
...
class SelectMultipleFilter extends Component {
  ...
  render() {
    ...
    return showAsPopup ? (
      ...
      <FilterPlain
        className={className}
        mainDivClass={id === 'TypeSearchFilters.filtersid' ? 'modalFilters' : ''}
        rootClassName={rootClassName}
        label={label}
