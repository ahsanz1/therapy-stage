...
export const handleSubmit = parameters => values => {
  ...
  const authorName = listing.author.attributes.profile.displayName
    ...
    .toLowerCase();
  async function main() {
    const promises = newSlots.map(async slots => {
      try {
        const initialValues = {
          listing,
          orderData: {
        ...
        await callSetInitialValues(setInitialValues, initialValues, saveToSessionStorage);

        await onInitializeCardPaymentData();
      } catch (error) {
        console.error('Error during processing slots:', error);
      }
    });
    await Promise.all(promises);
  }
  main();
  // Ensure that this code is inside an async function or a module that supports top-level await
