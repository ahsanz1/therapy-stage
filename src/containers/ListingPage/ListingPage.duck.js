...
export const SET_LINE_ITEMS = 'app/ListingPage/SET_LINE_ITEMS';
export const SET_ALL_LINE_ITEMS = 'app/ListingPage/SET_ALL_LINE_ITEMS';
export const SET_SELETED_TIME_SLOT = 'app/ListingPage/SET_SELETED_TIME_SLOT';
...
const calculateHoursDifference = (start, end) => {
  ...
  const hoursDifference = endDate?.diff(startDate, 'hours', true);
...
const listingPageReducer = (state = initialState, action = {}) => {
  ...
  switch (type) {
    ...
    case SET_LINE_ITEMS:
      return { ...state, lineItemsArray: payload };
    case SET_ALL_LINE_ITEMS:
      let totalQuantity = 0;
      let totalAmount = 0;

      payload.map((lineItem, i) => {
        const newValue = calculateHoursDifference(
          payload[i]?.attributes?.start,
          payload[i]?.attributes?.end
        );
        totalQuantity = totalQuantity + newValue;
        totalAmount = totalAmount + lineItem?.amount;
      });

      state.lineItems[0].lineTotal.totalAmount = new Money(
        totalAmount,
        state.lineItems[0].lineTotal.currency
      );
      state.lineItems[0].lineTotal.totalQuantity = totalQuantity;
      return { ...state, allLineItems: payload };
    case SET_SELETED_TIME_SLOT:
      return { ...state, selectedTimeSlot: payload };
...
export const setAllLineItems = lineItems => ({
  type: SET_ALL_LINE_ITEMS,
  payload: lineItems,
});
export const setSelectedTimeSlot = lineItems => ({
  type: SET_SELETED_TIME_SLOT,
  payload: lineItems,
});
...
export const fetchTransactionLineItems = ({ orderData, listingId, isOwnListing }) => (
  ...
  transactionLineItems({ orderData, listingId, isOwnListing })
    .then(response => {
      ...
      let totalQuantity = 0;
      let totalAmount = 0;
      getState()?.ListingPage?.allLineItems.map(lineItem => {
        totalQuantity = totalQuantity + lineItem?.quantity;
        totalAmount = totalAmount + lineItem?.amount;
      });
      lineItems[0].lineTotal.totalAmount = new Money(totalAmount, lineItems[0].lineTotal.currency);
      lineItems[0].lineTotal.totalQuantity = totalQuantity;
      // if (getState()?.ListingPage?.lineItems?.[0]?.lineTotal?.amount) {
      //   lineItems[0].lineTotal.amount =
      //     getState().ListingPage.lineItems[0].lineTotal.amount + lineItems[0].lineTotal.amount;
      //   lineItems[0].quantity.d[0] =
      //     Number(getState().ListingPage.lineItems[0].quantity) + Number(lineItems[0].quantity.d[0]);
      // }

      dispatch(fetchLineItemsSuccess(lineItems));
      // dispatch(setAllLineItems(lineItems));
    })
    .catch(e => {
