...
import FreeConsultation from './FreeConsultation.js';
import { productDetailEvent } from '../../util/datalayer-events.js';
...
export const ListingPageComponent = props => {
  ...
  const isFree = typeof window !== 'undefined' ? window.location.href.includes('/l/free') : null;
  ...
  React.useEffect(() => {
    if (currentListing?.id && !isFree ) {
      productDetailEvent('view_item', [], currentListing, intl);
    }
  }, []);
  ...
  } else if (!currentListing.id && !isFree) {
    // Still loading the listing
    return (
      <>
        <LoadingPage topbar={topbar} scrollingDisabled={scrollingDisabled} intl={intl} />
      </>
    );
  }
        ...
        <div className={css.contentWrapperForHeroLayout}>
          <div className={css.mainColumnForHeroLayout}>
            <div className={css.mobileHeading}>
              <H1 as="h1" className={css.orderPanelTitle}>
                <FormattedMessage
                  id="ListingPage.orderTitle"
                  values={{ title: authorDisplayName }}
                />
              </H1>
