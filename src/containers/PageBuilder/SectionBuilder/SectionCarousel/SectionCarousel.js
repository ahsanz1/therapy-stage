...
const SectionCarousel = props => {
  ...
  const backgroundColor = props?.appearance?.backgroundColor;
  ...
  const onSlideRight = e => {
    setCurrentSlide(prevSlide => (prevSlide + 1) % numberOfBlocks);
    e.target.focus();
  };
    ...
    >
      ...
      {hasBlocks ? (
        <div className={css.carouselContainer} id={sliderContainerId}>
          ...
          {console.log('numColumnsnumColumns', numColumns, getColumnCSS(numColumns))}
          <div className={`${getColumnCSS(numColumns)} ${css.slider}`} id={sliderId}>
            {duplicatedBlocks.slice(currentSlide, currentSlide + numColumns).map((block, index) => (
              <BlockBuilder
                key={index}
                mainSectionClassName={`section-${numColumns}`}
                rootClassName={`${css.block} ${sliderId}`}
