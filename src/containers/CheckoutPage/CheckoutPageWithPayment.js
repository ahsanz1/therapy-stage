...
import { beginCheckoutEvent } from '../../util/datalayer-events.js';
...
const getOrderParams = (pageData, shippingDetails, optionalPaymentParams, config) => {
  ...
  const bookingDates = bookingDatesMaybe(pageData?.orderData?.bookingDates)?.bookingDates;
  console.log(bookingDates);
  let bookingState = new Date(bookingDates?.bookingStart);
  let bookingEnd = new Date(bookingDates?.bookingEnd);

  let dateStart = bookingState;
  let dateEnd = bookingEnd;
  ...
  const orderParams = {
    ...
    allLineItem: pageData?.orderData?.allLineItem,
    multiBookings: multiBookingDates(pageData?.orderData?.allLineItem),
...
const handleSubmit = (values, process, props, stripe, submitting, setSubmitting) => {
  ...
  const lineItemArray = orderParams?.allLineItem;
  let resp = '';
  (async () => {
    for (let index = 0; index < lineItemArray.length; index++) {
      const lineItem = lineItemArray[index];
      const bd = lineItem?.attributes;
      let bookingState = new Date(bd?.start);
      let bookingEnd = new Date(bd?.end);
      ...
      const obj = {
        ...orderParams,
        bookingDates: { bookingStart: new Date(bd?.start), bookingEnd: new Date(bd?.end) },
        allLineItem: [lineItem],
        timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
        displayStart: {
          year: startYear,
          month: startMonth,
      ...
      resp = await processCheckoutWithPayment(obj, requestPaymentParams);
    }
    const { orderId, messageSuccess, paymentMethodSaved } = resp;
  ...
  //     savePaymentMethodFailed: !paymentMethodSaved,
  //   };
  //   console.log("checkingreponse", response)
  //   // setOrderPageInitialValues(initialValues, routeConfiguration, dispatch);
  //   // onSubmitCallback();
  //   // history.push(orderDetailsPath);
  // })
...
export const CheckoutPageWithPayment = props => {
  ...
  const tx =
    existingTransaction?.attributes?.lineItems?.length > 0
      ? existingTransaction
      : speculatedTransaction;
  let timeZone = listing?.attributes?.availabilityPlan?.timezone;
  ...
  const breakdown =
    orderData.allLineItem?.length > 0 ? (
      <OrderBreakdown
        ...
        // allLineItem={orderData.allLineItem}
      />
    ) : null;
  const totalPrice =
  ...
  const organizedName = listing?.author?.attributes?.profile?.displayName
    .replace(/\s+/g, '-')
    .toLowerCase();
  ...
  useEffect(() => {
    beginCheckoutEvent(orderData, listing, intl);
  }, []);
