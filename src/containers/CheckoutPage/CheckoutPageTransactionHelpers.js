...
import { ensureBooking, ensureStripeCustomer, ensureTransaction } from '../../util/data';
 ...
 * (or manibulate bookingStart and bookingEnd)
 *
 * @param {array} multiBookingDates
 * @returns array containing multiBookingDates or an empty array.
 */
export const bookingDatesMaybe = bookingDates => {
  return bookingDates ? { bookingDates } : {};
};

export const multiBookingDates = multiBookingDates =>
  multiBookingDates.map(data => data.attributes);
...
const persistTransaction = (order, pageData, storeData, setPageData, sessionStorageKey, fnParams = {}) => {
  // Store the returned transaction (order)
  if (order?.id) {
    // Store order.
    const { orderData, listing } = pageData;
    storeData(orderData, listing, order, sessionStorageKey);
    setPageData({ ...pageData, multiBookingDates: fnParams, transaction: order });
  }
};
...
export const processCheckoutWithPayment = (orderParams, extraPaymentParams) => {
  ...
  const bookingTx = ensureBooking(pageData?.orderData?.allLineItem);
  ...
  const fnRequestPayment = fnParams => {
    ...
    orderPromise.then(order => {
      // Store the returned transaction (order)
      persistTransaction(order, pageData, storeData, setPageData, sessionStorageKey, fnParams?.multiBookingDates);
    });
  ...
  const fnConfirmCardPayment = fnParams => {
    ...
    const response = hasPaymentIntentUserActionsDone
  ...
  //     : process.transitions.REQUEST_PAYMENT;
  // const isPrivileged = process.isPrivileged(requestTransition);
  // console.log('ASDASDzxczxqwe', orderParams, fnParams, processAlias, storedTx.id, requestTransition, isPrivileged);

  // orderParams?.multiBookings?.map(booking => {
  //     const bd = {
  //       bookingStart: new Date(booking?.start),
  //       bookingEnd: new Date(booking?.end),
  //     };
  //     const op = onInitiateOrder(
  //       { ...fnParams, bookingDates: { ...bd }, listingId: orderParams?.listingId?.uuid, allLineItem: pageData?.orderData?.allLineItem},
  //       'therapy-instant-booking/release-1',
  //       storedTx.id,
  //       requestTransition,
  //       isPrivileged
  //     );
