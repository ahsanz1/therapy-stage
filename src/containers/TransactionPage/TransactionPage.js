...
import { purchaseEvent } from '../../util/datalayer-events.js';
...
export const TransactionPageComponent = props => {
  ...
  useEffect(() => {
    return () => {
      typeof window !== undefined && window.sessionStorage.removeItem('is_reloaded');
    };
  }, []);

  useEffect(() => {
    const isReloaded =
      typeof window !== undefined && JSON.parse(window.sessionStorage.getItem('is_reloaded'));
    if (isReloaded) return;
    if (transaction && Object.keys(transaction).length > 0 && allLineItems.length > 0) {
      purchaseEvent(currentUser, transaction, allLineItems, intl);
      typeof window !== undefined && window.sessionStorage.setItem('is_reloaded', true);
    }
  }, [transaction, allLineItems, currentUser]);

  console.log('testinglog', allLineItems, transaction);
  ...
  const handleSubmitOrderRequest = values => {
    ...
    const newSlots = selectedTimeSlots.map(timeSlot => {
      let obj = {};
      const { bookingEndTime, bookingStartTime, startDate, endDate } = timeSlot;
      startDate
        ? (obj = {
            bookingDates: {
...
const mapStateToProps = state => {
  const allLineItems = state.ListingPage.allLineItems;
  const {
    fetchTransactionError,
    transitionInProgress,
    transitionError,
