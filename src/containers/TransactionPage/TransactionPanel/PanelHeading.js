...
const createListingLink = (listingId, label, listingDeleted, authorName, searchParams = {}, className = '') => {
  if (!listingDeleted) {
    const params = { id: listingId, slug: createSlug(label), organizedName: authorName };
...
const PanelHeading = props => {
  ...
  const listingLink = createListingLink(listingId, listingTitle, listingDeleted, authorName);
  const breakline = <br />;
