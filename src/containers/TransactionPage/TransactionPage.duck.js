...
export const makeTransition = (txId, transitionName, params) => (dispatch, getState, sdk) => {
  ...
  return sdk.transactions
    ...
    .then(async response => {
      const customer = response?.data?.data?.attributes?.protectedData;
      const provider = await sdk.currentUser.show();
      const dateDifference = Math.floor((params?.bookingEnd - params?.bookingStart) / 1000 / 60);
      const body = {
        meetingObj: {
          meeting_details_arr: [
            {
              topic: `${customer?.customerFirstName} &
                ${provider?.data?.data?.attributes?.profile?.firstName}`,
              type: 2,
              start_time: params?.bookingStart,
              timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
              duration: dateDifference,
              attendees: [
                {
                  email: provider?.data?.data?.attributes?.email,
                },
                {
      ...
      if (transitionName === 'transition/accept') {
        await fetch(`https://tib-backend.shopdev.co/create-calendar-invite`, {
          headers: {
