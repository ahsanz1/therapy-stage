...
export const AuthenticationForms = props => {
  ...
  const isLoginPage =
    typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('is_login');
  ...
  const tabs = isLoginPage === 'true' ? loginTab : _tabs;
      ...
      >
        <div className={css.privacyWrapper}>
          <PrivacyPolicyContent
            inProgress={tosFetchInProgress}
            error={tosFetchError}
            data={tosAssetsData?.[camelize(TERMS_CONDITION_ASSET_NAME)]?.data}
          />
        </div>
      </Modal>
    </Page>
